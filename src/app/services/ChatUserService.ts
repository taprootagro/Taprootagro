...
//   3. Register the user on the IM provider via Supabase Edge Function
//   4. Track registration status per provider to avoid duplicate calls
//
// Registration flow:
//   Frontend → Edge Function → IM Provider REST API (create user)
//   The Edge Function holds the API Key/Secret, frontend only sends user info.
// ============================================================================
...
class ChatUserService {
  ...
  /**
   * Register user on the configured IM provider via Edge Function.
   * 
  ...
  async registerOnProvider(): Promise<{ success: boolean; error?: string }> {
    ...
    // Real backend call
    const endpoint = `${cfg.supabaseUrl}/functions/v1/${cfg.edgeFunctionName}/register`;

...
// ---- Backend auth helper ----
// When Supabase backend is enabled, call Edge Function /auth endpoint
// to verify credentials and get a server-assigned user ID.
// Falls back to local ID generation when backend is unavailable.
...
async function authenticateViaBackend(
  ...
  try {
    ...
    if (!saved) return { success: false, error: "no_config" };

    const parsed = JSON.parse(saved);
    const bpc = parsed.backendProxyConfig;
    if (!bpc?.enabled || !bpc?.supabaseUrl) {
      return { success: false, error: "backend_disabled" };
    }

    const endpoint = `${bpc.supabaseUrl}/functions/v1/${bpc.edgeFunctionName || "chat-proxy"}/auth`;
    ...
    console.warn("[Login] Backend auth failed, falling back to local:", err);
...
export function LoginPage() {
  ...
  // Unified login handler: tries server auth first, falls back to local
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result.success && result.userId) {
        // Tier 1: Server-assigned ID — secure
        setServerUserId(result.userId);
        console.log(`[Login] Server-assigned user ID: ${result.userId}`);
      } else {
        // Tier 2: Local fallback — demo/preview only
        console.log(`[Login] Using local ID fallback (${result.error})`);
      }
  ...
  const handleSocialLogin = async (platform: string) => {
    ...
    setIsLoading(true);
    try {
      // Social login: try server auth with platform as method
      const result = await authenticateViaBackend(platform as any, platform, "");
      if (result.success && result.userId) {
        setServerUserId(result.userId);
      }
      setUserLoggedIn(true);
